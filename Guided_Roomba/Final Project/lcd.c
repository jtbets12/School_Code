...
void lcd_rotatingBanner(char dispText[]){
    ...
    int i = 0;
    uint32_t millis = 300;
    int maxVal = strlen(dispText) + 40;
    char space = ' ';


    while(1)    {

        if (i >= maxVal) {                      //when the string text has "exited" the left side of the screen, the process will reset
            lcd_init();
            i = 0;
            timer_waitMillis(millis);
        }

        else if (i < 20)    {                   //this process begins when there are spaces that need to be displayed to the left of the string
            int j = 0;
            int k = 0;
            for(j = (20 - i); j > 0; j--)  {    //prints spaces in the first j-i positions on the LCD, the number of spaces decrease as i increases
                lcd_putc(space);

            }

            for(k = 0; k < i; k++) {            //prints chars at position k of dispText so long as k < the size of dispText
                if(k < strlen(dispText))    {   //prints the char in position k, or prints a space if k >= the size of dispText
                    lcd_putc(dispText[k]);
                }   else    {
                    lcd_putc(space);
                }
        ...
        else {                                          //begins when the text on the rotating banner has reached the left side of the screen
            int m = 0;

            for(m = i - 20; m < i; m++) {
                if(m >= strlen(dispText) + 20)   {      //if the banner has exited the left size of the screen, i is set to maxVal forcing a reset,
                                                        // or prints the chars of dispText at position m, or it prints spaces in the remaining positions
                                                        // on the LCD if m >= the size of dispText
                    i = maxVal;
                }   else if(m < strlen(dispText))    {
                    lcd_putc(dispText[m]);
                }   else    {
                    lcd_putc(space);
                }
